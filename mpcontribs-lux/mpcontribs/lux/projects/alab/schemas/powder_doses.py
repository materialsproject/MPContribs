...
class PowderItem(BaseModel, extra="forbid"):
    ...
    PowderName: str | None = Field(
        default=None,
        description="Name of the powder"
    )
    TargetMass: float | None = Field(
        default=None,
        description="Target mass in mg",
        ge=0
    )
    Doses: list[dict] | None = Field(
        default=None,
        description="List of individual doses"
    )
...
class PowderDose(BaseModel, extra="forbid"):
    ...
    powder_name: str = Field(
        description="Name of the powder material"
    )
    
    target_mass: float = Field(
        description="Target mass in mg",
        ge=0
    )
    
    actual_mass: float = Field(
        description="Actual mass dispensed in mg",
        ge=0
    )
    
    accuracy_percent: float = Field(
        description="Dosing accuracy (actual/target * 100)"
    )
    
    dose_sequence: int = Field(
        description="Sequence number within the experiment",
        ge=0
    )
    
    head_position: int | None = Field(
        default=None,
        description="Dispenser head position"
    )
    
    dose_timestamp: str | None = Field(
        default=None,
        description="Timestamp of the dose event"
    )
