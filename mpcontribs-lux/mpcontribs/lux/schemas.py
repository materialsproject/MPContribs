...
NON_DATA_FIELDS = {"identifier","formula","units","aliases","structures","attachments"}
...
class ContributionRecord(BaseModel):
    ...
    units : dict[str,str] = Field({},description="mapping of column names to units.",exclude=True)
    structures : list[StructureType] = Field([], description="Structures associated with this entry.")
    attachments : list[AttachmentRecord] = Field([])
    aliases : dict[str,str] = Field({}, description="Aliases of fields to use when generating column names.", exclude=True)
    ...
    def to_contribs_entry(self) -> dict[str,Any]:
        ...
        return {
            ...
            "data": {
                k : f"{getattr(self,k,None)} {self.units.get(k,'')}".strip()
                for k in set(
                    self.__class__.model_fields
                ).difference(NON_DATA_FIELDS)
            },
    ...
    def columns(self) -> dict[str,str]:
        return {
            k : self.units.get(k) or (
                "" if any(t in get_args(field.annotation) for t in (int,float)) else None
            )
