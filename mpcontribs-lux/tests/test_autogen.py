...
def test_schema_generation(test_dir):
    ...
    with gzip.open(
        (import_module_path("mpcontribs.lux") / ".." /".."/ "examples" /"cubic_solid_expt_data.json.gz").resolve(),
        "rb"
    ) as source_file:
        temp_file.write(
            json.dumps(
                json.load(source_file)["data"]
            ).encode()
        )

    temp_file.seek(0)

    schemer = SchemaGenerator(file_name = temp_file.name,)
    ...
    assert all(
        model.model_fields[k].annotation == anno for k, anno in expected_field_types.items()
    )

    new_desc = {"b0": "Bulk modulus", "cif": "CIF"}
    with open(test_py_file := Path("./test_w_name_and_anno.py").resolve(),"wt") as py_temp:
        py_temp.write(
            schemer.schema(model_name = "TestClass", descriptions=new_desc)
        )
    ...
    assert set(model.model_fields) == expected_model_fields
    assert all(model.model_fields[k].description == desc for k, desc in new_desc.items())
    assert all(
        model.model_fields[k].annotation == anno for k, anno in expected_field_types.items()
    )
