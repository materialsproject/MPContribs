...
def main_page(request):
	if request.method == "GET":
		...
		if mat_form.is_valid():
			...
			query = reduce(lambda x, y: x | y, [Q(__raw__={("primitive_structure.composition."+symbol):{"$exists":"true"}}) for symbol in elements])
			...
			for q in queryset:
				...
				table.append({
					...
					'total_energy_error':dash_if_empty(round(en_fit['c0'][1]*H2eV,5)),
					'material_id': str(q.pk)})

			return render(request,'materialqueryresponse.html',{'table':table, 'form':mat_form})
	else:
