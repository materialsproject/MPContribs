...
def material_overview(request, material_id):
    ...
    provenance = [
        {
            ...
            "dft_source": {
                ...
                "basis": mark_safe(
                    "</br>".join(
                        [
                            "&emsp;" + b.strip() if b[0] == " " else b.strip()
                            for b in r.trial_source.trial_source.basis_set.split("\n")
                        ]
                    )
                ),
            },
    ...
    attributes = {
        ...
        "cif": mark_safe(
            qmcdbset.primitive_structure.cif.replace("'", "\\'").replace("\n", "\\n")
        ),
    }
    ...
    total_energies = str(
        [
            [1 / r.structure.nfu, round(r.total_energy * H2eV / r.structure.nfu, 5)]
            for r in records
        ]
    )
    ...
    total_energy_errors = str(
        [
            [
                1 / r.structure.nfu,
                round(
                    (r.total_energy - r.total_energy_error / 2)
                    * H2eV
                    / r.structure.nfu,
                    5,
                ),
    ...
    ## PROPERTIES ##
    y_int = {
        "value": round(fit["c0"][0] * H2eV, 5),
        "error": round(fit["c0"][1] * H2eV, 5),
    }
    formation_enthalpy = qmcdbset.formation_enthalpy
    if formation_enthalpy != None:
        formation_enthalpy = {
            "value": round(formation_enthalpy[0] * H2eV, 5),
            "error": round(formation_enthalpy[1] * H2eV, 5),
        }
    else:
        formation_enthalpy = {"value": None, "error": None}
    attributes["infinite_limit"] = {
        "total_energy": y_int,
        "formation_enthalpy": formation_enthalpy,
    }

    return render(
        request, "materialoverview.html", {"attributes": attributes, "chart": cht}
    )
