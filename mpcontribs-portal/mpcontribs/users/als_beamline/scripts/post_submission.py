...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "cids_file",
        type=str,
        help="""Path to file containing contribution IDs
    ...
    parser.add_argument(
        "--dev",
        action="store_true",
        help="""Run in dev mode based on MPFile
    ...
    for i in range(0, n, 1):
        for k in range(n - i + 1, 0, -1):
            ...
            gx.append(x0 + x2 / 2.0)  # NOTE x0 might need to be replace with x1
    ...
    for fldidx, fields_str in enumerate(fields_strings):
        ...
        for elidx, elem in enumerate(chemsys[:-1]):
            ...
            for idx, (comp, cid) in enumerate(comps_cids):
                ...
                x.append(x0 + x2 / 2.0)  # NOTE x0 might need to be replace with x1
                y.append(x2 * math.sqrt(3.0) / 2.0)
                if fldidx < 3:
                    zval = mpfile.document[comp_str]["{} XMCD".format(elem)][fields[0]][
                        fields[1]
                    ]
                    if fldidx == 0:
                        norms[elem] = zval
