...
def get_preedge_spectrum(process_parameters, xmcd_data):
    preedge = (
        map(float, process_parameters["preedge_range"].split())
        if "preedge range" in process_parameters
        else [0, 10000]
    )
...
def normalize_XAS_minmax(
    xmcd_data, scanparams=None, process_parameters=None, process_number=-1
):
...
def xas_xmcd_minmax(
    ...
    energy_range = (
        map(float, process_parameters["energy_range"].split())
        if "energy range" in process_parameters
        else [0, 10000]
    )
    energy = xmcd_data["Energy"]
...
def get_xmcd(xmcd_data, scanparams=None, process_parameters={}, process_number=-1):
    ...
    if xas_plus.shape != xas_minus.shape:
        print(
            "xas_plus & xas_minus not equal. Using xas_plus. Shapes: {0}, {1}".format(
