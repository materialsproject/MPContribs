...
class TestDeleteProject:
    def test_delete_returns_cascade_summary(self, client, project_service):
        project_service.delete_one.return_value = DeleteSummary({"projects": 1, "contributions": 3})
        r = client.delete("/api/v1/projects/mp-sample", headers=AUTHED_HEADERS)
        assert r.status_code == 200
        # The body is the per-type count of everything the cascade removed.
        assert r.json() == {"projects": 1, "contributions": 3}

    def test_id_forwarded_to_service(self, client, project_service):
        project_service.delete_one.return_value = DeleteSummary.of("projects", 1)
