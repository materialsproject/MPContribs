...
class ComponentService[
    ...
    async def get_many(
        ...
        """Return a scoped, filtered, cursor-paginated page of components. See ``get_many``."""
        return await self._components.get_many(pagination=pagination, filter=filter, fields=fields)

    async def get_by_id(self, id: str, fields: frozenset[str] | None) -> TDoc | TOut | None:
        """Find a single component by id, scoped to the current user. See ``get_component_by_id``."""
    ...
    async def patch_by_id(self, id: str, update: TPatch) -> TDoc:
        """Partially update a component by id, scoped to the current user. See ``patch_component_by_id``."""
