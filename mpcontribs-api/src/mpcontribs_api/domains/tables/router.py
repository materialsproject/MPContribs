...
async def delete_many(service: TableServiceDep, filter: TableFilter = FilterDepends(TableFilter)) -> DeleteResult:
    return await service.delete_many(filter=filter)


@router.delete("/{id}", dependencies=[Depends(require_user)])
async def delete_one(service: TableServiceDep, id: str) -> DeleteResult:
